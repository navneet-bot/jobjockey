import { getInternshipApplications } from "@/actions/applicationActions";
...
export default async function AdminInternshipApplicationsPage({ params }: { params: Promise<{ id: string }> }) {
    const { id } = await params;
    const internship = await getSingleInternship(id);

    if (!internship) notFound();

    const apps = await getInternshipApplications(id);
            ...
            <div className="flex items-center justify-between">
                <div className="flex flex-col gap-2">
                    ...
                    <GradientHeader
                        ...
                        subtitle={`Reviewing applications for this internship at ${internship.company}`}
                    />
                </div>
            </div>
