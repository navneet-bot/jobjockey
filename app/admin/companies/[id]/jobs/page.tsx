        ...
        <div className="flex flex-col gap-10">
            <div className="flex items-center justify-between">
                ...
                <Link
                    ...
                    className="px-4 py-2 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl hover:bg-black/10 dark:hover:bg-white/10 transition-colors text-sm font-medium text-[var(--text-main)] dark:text-white"
                >
                    Back to Details
                </Link>
            </div>
                    ...
                    jobs.map((job) => (
                        <GlassCard key={job.id} className="p-6 flex flex-col gap-6 hover:border-[var(--primary)]/30 transition-all group">
                            <div className="flex flex-col gap-2">
                                <h3 className="font-bold text-lg text-[var(--text-main)] dark:text-white dark:group-hover:text-[var(--primary)] transition-colors">{job.title}</h3>
                            ...
                            <div className="flex flex-col gap-3">
                                ...
                                <Link
                                    ...
                                    className="flex items-center gap-3 text-sm text-[var(--text-main)] dark:text-white font-semibold dark:hover:text-white hover:text-[#111827] transition-colors"
                                >
                                    <Users className="w-4 h-4 text-[var(--text-main)] dark:text-[var(--primary)]" />
                            ...
                            <Link
                                href={`/admin/jobs/${job.id}/applications`}
                                className="mt-2 w-full py-3 bg-black/5 dark:bg-white/5 border border-black/10 dark:border-white/10 rounded-xl hover:bg-[#111827] dark:hover:bg-[var(--primary)] hover:text-white dark:hover:text-black transition-all flex items-center justify-center gap-2 font-bold text-sm text-[var(--text-main)] dark:text-white"
                            >
                                View Applications
